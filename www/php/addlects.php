<?php
// input file is a series of lines, one field per line with a blank line between lectures
//
// each lecture is:
// courseid
// title
// lecturer (exact full name match required)
// path to thumbnail
// 
...
foreach ($indata as $line) {
  ...
  if ($index == 4) {
    ...
    if ($q->rowCount() != 1) die("More than one professor with the same name @line:$ln");
    $profid = $q->fetchColumn();
    $row[2] = $profid;
      
    // have all fields, so insert
    $sql = "insert into lecture (courseid, title, lecturerid, thumb) ".
      "values (?,?,?,?)";
    $q = $db->prepare($sql);
    if (!$q->execute($row)) {
